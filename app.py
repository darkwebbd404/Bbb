...
def generate_custom_password(random_length=9):
    ...
    return f"MRKALPHA-{random_part}-X64"
...
def get_user_inputs():
    ...
    print(f"{bold}{green}=== Free Fire Account Generator ==={Fore.RESET}")
    print(f"{bold}{purpel}Welcome! Let's create some accounts.{Fore.RESET}\n")
    ...
    account_name = input(f"{bold}{red}[{lg}+{red}]{red} Enter account name prefix (default: MRKALPHAX64) : {Fore.RESET}").strip()
    if not account_name:
        account_name = "MRKALPHAX64"
